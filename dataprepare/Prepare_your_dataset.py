...
train_number = 56 # Randomly assign the number of images for generating the training set.
val_number = 16   # Randomly assign the number of images for generating the validation set.
test_number = 8  # Randomly assign the number of images for generating the test set.
all = int(train_number) + int(val_number) + int(test_number)

############################################################# Prepare your data set #################################################
Tr_list = glob.glob("training/image"+'/*.png')   # Images storage folder. The image type should be 24-bit png format.
...
for idx in range(len(Tr_list)):
    ...
    img2 = sc.imread(Ms_List[idx])
    img2 = np.double(sc.imresize(img2, [height, width], interp='nearest'))
